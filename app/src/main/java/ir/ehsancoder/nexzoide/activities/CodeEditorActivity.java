...
import ir.ehsancoder.nexzoide.G4Compiler;
import ir.ehsancoder.nexzoide.IdeEditor;
import ir.ehsancoder.nexzoide.JavaCcComplierImpl;
import ir.ehsancoder.nexzoide.R;
import ir.ehsancoder.nexzoide.adapter.SyspiarAdapter;
import ir.ehsancoder.nexzoide.adapter.ToolbarListFileAdapter;
import ir.ehsancoder.nexzoide.config.JavaToGsonHelper;
import ir.ehsancoder.nexzoide.databinding.Antcomp8lerBinding;
import ir.ehsancoder.nexzoide.enums.CompilerModel;
import ir.ehsancoder.nexzoide.enums.Mode;
import ir.ehsancoder.nexzoide.layoutmanager.LogCatBottomSheet;
import ir.ehsancoder.nexzoide.marco.ColorView;
import ir.ehsancoder.nexzoide.marco.FactoryCodeError;
import ir.ehsancoder.nexzoide.marco.nexzoWebEditorSearch;
import ir.ehsancoder.nexzoide.marco.KotlinCompilerImpl;
import ir.ehsancoder.nexzoide.marco.WallpaperParallaxEffect;
import ir.ehsancoder.nexzoide.model.EditorViewModel;
import ir.ehsancoder.nexzoide.navigator.EditorRoaderFile;
import ir.ehsancoder.nexzoide.project.JavaCompilerBeta;
import ir.ehsancoder.nexzoide.project.ProjectManager;
import ir.ehsancoder.nexzoide.tasks.FileChangeReceiver;
import ir.ehsancoder.nexzoide.tasks.SecurePrefs;
import ir.ehsancoder.nexzoide.tasks.app.SassForAndroid;
...
import ir.ehsancoder.nexzoide.utils.CompilerUtils;
import ir.ehsancoder.nexzoide.utils.DataUtil;
import ir.ehsancoder.nexzoide.widget.BlurImage;
import ir.ehsancoder.nexzoide.widget.ExrtaFab;
...
public class CodeEditorActivity extends BaseCompat {
  ...
  private void initialize(Bundle _savedInstanceState) {
    ...
    nexzo_searchs = findViewById(R.id.editor_ser);
    nexzo_searchs.hide();
    ...
    nexzoIcon = findViewById(R.id.icon_backgroundnexzo);
    ...
    nexzo_searchs.bindEditor(editor);
    nexzo_searchs.setCallBack(
        new nexzoWebEditorSearch.onViewChange() {
  ...
  private void initializeLogic() {
    ...
    FileUtil.writeFile(
        "/storage/emulated/0/nexzoWebIDE/ehsancoder/openFile.json", shp.getString("path", ""));
    ...
    if (setfont.contains("mfont")) {
      if (!FileUtil.isFile(setfont.getString("mfont", ""))) {
        editor.setTypefaceText(Typeface.createFromAsset(getAssets(), "nexzofont.ttf"));
        editor.setTypefaceLineNumber(Typeface.createFromAsset(getAssets(), "nexzofont.ttf"));
        DataUtil.showMessage(getApplicationContext(), "Custom Font Not Found");
        titleauthor.setTypeface(
            Typeface.createFromAsset(getAssets(), "fonts/nexzofont.ttf"), Typeface.NORMAL);
    ...
    BlurImage.setBlurInWallpaperMobile(this, data, nexzoIcon);
  ...
  public void setCloseall() {
    ...
    FileUtil.deleteFile("/storage/emulated/0/nexzoWebIDE/ehsancoder/openFile.json");
    finish();
  }
  ...
  public void setCloseother() {
    ...
    FileUtil.writeFile(
        "/storage/emulated/0/nexzoWebIDE/ehsancoder/openFile.json", shp.getString("pos_path", ""));
    shp.edit().putString("path", new Gson().toJson(tabs_listmap)).apply();
  }
