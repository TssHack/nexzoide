/**
 * Comment by nexzo ide
 *
 * @author: ehsancoder
 */
package ir.ehsancoder.nexzoide.activities;

import com.ehsancoder.jgit.search.SearchCallBack;
...
import ir.ehsancoder.nexzoide.Store.StoreAcitvity;
import ir.ehsancoder.nexzoide.adapter.FileManagerAd;
import ir.ehsancoder.nexzoide.adapter.ViewType;
import ir.ehsancoder.nexzoide.compressor.TarGzExtractor;
import ir.ehsancoder.nexzoide.compressor.XmlToSvg;
import ir.ehsancoder.nexzoide.compressor.ZxExtractor;
import ir.ehsancoder.nexzoide.databin.FileMaker;
import ir.ehsancoder.nexzoide.databinding.FiledirBinding;
import ir.ehsancoder.nexzoide.marco.binder.BindJpegWall;
import ir.ehsancoder.nexzoide.marco.editorface.ClassNodePaserImpl;
import android.util.Log;
import ir.ehsancoder.nexzoide.filehelper.CreatorModule;
import ir.ehsancoder.nexzoide.filehelper.FactoryModelProject;
import ir.ehsancoder.nexzoide.git.GitListSheet;
import ir.ehsancoder.nexzoide.interfaces.FileCallBack;
import ir.ehsancoder.nexzoide.marco.*;
import ir.ehsancoder.nexzoide.model.IconShop;
import ir.ehsancoder.nexzoide.model.ListSheet;
import ir.ehsancoder.nexzoide.model.ZipFileShow;
import ir.ehsancoder.nexzoide.model.dataobject.ShortcutInfoImpl;
import ir.ehsancoder.nexzoide.project.ProjectMaker;
import ir.ehsancoder.nexzoide.recyclerview.RecyclerViewHelper;
import ir.ehsancoder.nexzoide.services.FileEventUser;
import ir.ehsancoder.nexzoide.tasks.FileWatcher;
import ir.ehsancoder.nexzoide.tasks.FileWatcherService;
import ir.ehsancoder.nexzoide.tasks.FileWatcherServiceConnection;
import ir.ehsancoder.nexzoide.terminal.TerminalActivity;
import ir.ehsancoder.nexzoide.utils.*;
import ir.ehsancoder.nexzoide.utils.ObjectUtils;
import ir.ehsancoder.nexzoide.utils.DialogUtil;
import ir.ehsancoder.nexzoide.utils.VectorHelper;
import ir.ehsancoder.nexzoide.widget.nexzoWebMaterialDialog;
import ir.ehsancoder.nexzoide.widget.component.fastscrollcompat.FastScrollerBuilder;
...
import com.ehsancoder.jgit.GitClone;
import com.ehsancoder.jgit.GsonToClass;
import com.ehsancoder.jgit.childer.TextFindListener;
import ir.ehsancoder.prograsssheet.MusicSheet;
import java.util.concurrent.ExecutorService;
import java.util.concurrent.Executors;
import java.util.stream.Collectors;
import ehsan.coder.appuploader.data.SnippetManagerImpl;
import ehsancoder.nexzoide.androidtools.r8.android.R8Tools;
import storage.sdcard.SdCardUtil;
import com.ehsancoder.jgit.childer.TextFind;
import java.io.File;
import java.util.*;
import com.ehsancoder.jgit.childer.LayoutBinder;
...
public class FileManagerActivity extends BaseCompat
        ...
        new RequestNetwork.RequestListener() {
          ...
          public void onResponse(String _param1, String response, HashMap<String, Object> _param3) {
            ...
            if (!upfile.get((int) 0).get("Version").toString().equals(version)) {
              /// AddDialog
              var di = new nexzoWebMaterialDialog(FileManagerActivity.this);
  ...
  private void initStartApp() {
    ...
    CheckNewVersion.startRequestNetwork(
        ...
        "https://raw.githubusercontent.com/tsshack/tsshack/main/log.json",
        "",
        UpdateCheck);
    try {
      var pInfo =
  ...
  public void FolderMaker() {
    var dialog =
        new nexzoWebMaterialDialog(FileManagerActivity.this)
          ...
          positive.setOnClickListener(
              (vftrororocjj) -> {
                ...
                if (editor.getText().toString().isEmpty()) {
                  ...
                  try {
                    ehsanMacroFileUtil.createDirectory(
                        Folder.concat("/".concat(CreateFolder.concat("/"))),
                        new ehsanMacroFileUtil.OnFileOperationListener() {
  ...
  public void _themeinstall(
    ...
    if (_list.get((int) _pos).get(_str).toString().endsWith(".aa")
      ...
      if (FileUtil.isFile("/storage/emulated/0/nexzoWebIDE/theme/nexzoThemeapp.nexzo")) {
            ...
            (p, d) -> {
              try {
                new net.lingala.zip4j.ZipFile(_list.get((int) _pos).get(_str).toString())
                    .extractAll("/storage/emulated/0/nexzoWebIDE/theme/");
                ...
                if (output.getText().toString().isEmpty()) {
                  ...
                  new AsyncTask<String, String, String>() {
                    ...
                    protected String doInBackground(String... params) {
                      ...
                      pv.add("/storage/emulated/0/nexzoWebIDE/android/android.jar");
                      pv.add("--outputdir");
  ...
  public void _dataOnClickItemList(int _pos) {
    ...
    if (staticstring.endsWith(".nexzo")) {
      loadThemenexzo();
    }
    if (staticstring.endsWith(".xml")) {
      loadVector(newpos);
    }
    if (staticstring.endsWith(".ehsan")) {
      SendDataFromCodeEditor(newpos, "path", files, newlistmap);
    }
  ...
  public void setBackupTheme() {
    if (FileUtil.isFile("/storage/emulated/0/nexzoWebIDE/theme/theme.AA")) {
    } else {
      try {
        var zip4 =
            new net.lingala.zip4j.ZipFile(
                new File(
                    "/storage/emulated/0/nexzoWebIDE/" + DataUtil.getRandom(1, 200) + "theme.AA"));
        List<File> filesToAdd = new ArrayList<>();
        var iconPath = getFilesDir().getAbsoluteFile() + "/icon.png";
        filesToAdd.add(new File(iconPath));
        filesToAdd.add(new File("/sdcard/nexzoWebIDE/theme/nexzoThemeapp.nexzo"));
    ...
    sh.setOnItemClickLabe(
        (pos333) -> {
          switch (pos333) {
            ...
            case 5:
              {
                var di1 = new nexzoWebMaterialDialog(FileManagerActivity.this);
                ...
                di1.setMessage("Type Name Folder + name ehsan.ehsan.app.txt");
