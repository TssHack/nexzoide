...
import ir.ehsancoder.nexzoide.nexzoIdeAppLoader;
import ir.ehsancoder.nexzoide.R;
import ir.ehsancoder.nexzoide.adapter.ListAppIconAd;
import ir.ehsancoder.nexzoide.config.AppIconManager;
import ir.ehsancoder.nexzoide.config.PrfnsUtil;
import ir.ehsancoder.nexzoide.enums.EffectTypeManager;
import ir.ehsancoder.nexzoide.enums.ErudaThemeManager;
import ir.ehsancoder.nexzoide.enums.ErudaThemes;
import ir.ehsancoder.nexzoide.git.GitHubProfileView;
import java.util.Arrays;
import ir.ehsancoder.nexzoide.filehelper.IconCursorImpl;
...
import ir.ehsancoder.prograsssheet.LayoutSheetEditText;
import ir.ehsancoder.prograsssheet.perfence.PerfenceLayoutSubTitle;
import ir.ehsancoder.prograsssheet.perfence.PreferenceSwitchGroup;
...
public class SettingAppActivity extends BaseCompat {
  ...
  private void initialize(Bundle _savedInstanceState) {
    ...
    themecustom.setOnClickListener(
        v -> {
          customDataRow(
              "Custom Theme", "Select Theme in format .nexzo", "themes", thememanagersoft);
        });
    ...
    windowsize.setOnClickListener(
        v -> {
          var di = new nexzoWebMaterialDialog(SettingAppActivity.this);
    ...
    breaklevelmodel.setSwitchChangedListener(
        (bin, is) -> {
          if (is) nexzoIdeAppLoader.getPrefManager().edit().putBoolean("breaks", true).apply();
          else nexzoIdeAppLoader.getPrefManager().edit().putBoolean("breaks", false).apply();
        });
  ...
  public void _seechackswich() {
    ...
    if (nexzoIdeAppLoader.getPrefManager().getBoolean("breaks", false) == true) {
  ...
  public void _blursize() {
    ...
    ir.ehsancoder.nexzoide.layoutmanager.SliderCompat slider = dialogview.findViewById(R.id.slider);
  ...
  public class Recyclerview2Adapter extends RecyclerView.Adapter<Recyclerview2Adapter.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder _holder, final int _position) {
      ...
      if (_position == 4) {
        ...
        imageview1.setImageResource(R.drawable.nexzotheme);
        _view.setOnClickListener(
            _dddrr_ -> {
              ...
              dialog.setPositiveButton(
                  "OK",
                  (position, theme) -> {
                    nexzoIdeAppLoader.getThemeEngine().setStaticTheme(theme);
                    themeEngine.setStaticTheme(theme);
                    ((nexzoIdeAppLoader) getApplicationContext()).onThemeChange();
                    recreate();
                    finishAffinity();
                  });
              ...
              dialog.setNeutralButton(
                  ...
                  (param1, param2) -> {
                    ...
                    nexzoIdeAppLoader.getThemeEngine().resetTheme();
                    ((nexzoIdeAppLoader) getApplicationContext()).onThemeChange();
                    recreate();
                  });
              dialog.create().show();
