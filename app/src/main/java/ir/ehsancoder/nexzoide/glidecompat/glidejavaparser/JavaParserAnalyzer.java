package ir.ehsancoder.nexzoide.glidecompat.glidejavaparser;
...
public class JavaParserAnalyzer implements JavaCodeAnalyzer {
  ...
  private boolean isStandardJavaClass(ClassOrInterfaceDeclaration clazz) {
    try {
      InputStream jsonFile = nexzoIdeAppLoader.getContext().getAssets().open("javaclass.json");
  ...
  private boolean isStandardAndroidViews(ClassOrInterfaceDeclaration clazz) {
    try {
      InputStream jsonFile = nexzoIdeAppLoader.getContext().getAssets().open("androidviews.json");
