package ir.ehsancoder.nexzoide.navigator;
...
import ir.ehsancoder.nexzoide.utils.FileCompatApi28;
...
public class EditorRoaderFile {
  ...
  public static void RuningTask(IdeEditor editor, ExrtaFab fab, String path, ProgressBar bar) {
    ...
    if (path.endsWith(".css")) {
      ...
      fab.postDelayed(fab::show, 400);
    } else if (path.endsWith(".nexzo")) {
      ReadFileCompat(editor, path, bar);
      editor.setEditorLanguage(new nexzolangs());
    } else if (path.endsWith(".ehsan")) {
      ReadFileCompat(editor, path, bar);
      editor.setEditorLanguage(new ehsanLang());
