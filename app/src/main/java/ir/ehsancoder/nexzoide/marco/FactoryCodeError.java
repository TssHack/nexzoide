package ir.ehsancoder.nexzoide.marco;
...
import com.ehsancoder.jgit.AsyncTaskCompat;
...
import io.github.rosemoe.sora.langs.nexzothemelang.nexzolang.nexzolangs;
...
import io.github.rosemoe.sora.langs.ehsanlang.ehsanLang;
...
public class FactoryCodeError {
   ...
   * شدید است من سیع میکنم در نکزو ide ویژگی های مفید پیاده سازی کنم
   */
   ...
   */
  public void run() {
    if (nexzoIdeAppLoader.getAnalyzercod().getBoolean("Analyzercod", false) == true) {
      ...
      if (editor.getEditorLanguage() instanceof CSS3Language) {
        ...
        dart();
      } else if (editor.getEditorLanguage() instanceof nexzolangs) {
        Iconnotfound();
        ...
        kt();
      } else if (editor.getEditorLanguage() instanceof ehsanLang) {
        Iconnotfound();
