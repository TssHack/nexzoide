...
public class AssetsSoft {
  ...
  public void z() {
    try {
      InputStream temp7zStream = nexzoIdeAppLoader.getContext().getAssets().open("python.7z");
      File file =
          new File(nexzoIdeAppLoader.getContext().getFilesDir().getAbsolutePath() + "/python.7z");
      file.createNewFile();
      Files.copy(temp7zStream, file.toPath(), StandardCopyOption.REPLACE_EXISTING);
      unzipCompat(
          file.getAbsolutePath(), nexzoIdeAppLoader.getContext().getFilesDir().getAbsolutePath());
      file.delete();
      temp7zStream.close();
      // Toast.makeText(nexzoIdeAppLoader.getContext(), "unZipet " + file.getName(), 2).show();
    } catch (IOException err) {
      Toast.makeText(nexzoIdeAppLoader.getContext(), err.getMessage(), 2).show();
    }
  }

  public void php() {
    try {
      InputStream temp7zStream = nexzoIdeAppLoader.getContext().getAssets().open("libs.7z");
      File file =
          new File(nexzoIdeAppLoader.getContext().getFilesDir().getAbsolutePath() + "/libs.7z");
      file.createNewFile();
      Files.copy(temp7zStream, file.toPath(), StandardCopyOption.REPLACE_EXISTING);
      unzipCompat(
          file.getAbsolutePath(), nexzoIdeAppLoader.getContext().getFilesDir().getAbsolutePath());
  ...
  public void unZipGcc() {
    try {
      InputStream temp7zStream = nexzoIdeAppLoader.getContext().getAssets().open("gcc.zip");
      File file =
          new File(nexzoIdeAppLoader.getContext().getFilesDir().getAbsolutePath() + "/gcc.zip");
      file.createNewFile();
      Files.copy(temp7zStream, file.toPath(), StandardCopyOption.REPLACE_EXISTING);
      unzip(file.getAbsolutePath(), nexzoIdeAppLoader.getContext().getFilesDir().getAbsolutePath());
      file.delete();
      temp7zStream.close();
      //	Toast.makeText(nexzoIdeAppLoader.getContext(), "unZipet " + file.getName(), 2).show();
    } catch (IOException err) {
  ...
  protected MaterialShapeDrawable db() {
    ...
    mt.setFillColor(
        ColorStateList.valueOf(
            MaterialColors.getColor(nexzoIdeAppLoader.getContext(), ObjectUtils.Back, 0)));
    mt.setStroke(
        1, MaterialColors.getColor(nexzoIdeAppLoader.getContext(), ObjectUtils.TvColor, 0));
    return mt;
  }

  private void unzipCompat(String input, String output) {
    AlertDialog dialog =
        new MaterialAlertDialogBuilder(nexzoIdeAppLoader.getContext())
            .setTitle("")
                ...
                Z7Extractor.extractFile(
                    ...
                    new com.hzy.lib7z.IExtractCallback() {
                      ...
                      public void onProgress(String arg0, long arg1) {
                        ((AppCompatActivity) nexzoIdeAppLoader.getContext())
                            .runOnUiThread(
                                new Runnable() {
                                  @Override
                                  public void run() {
