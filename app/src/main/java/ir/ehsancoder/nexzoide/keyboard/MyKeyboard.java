...
public class MyKeyboard extends Keyboard {
  ...
  public Key createKeyFromXml(
    ...
    SharedPreferences sharedPref =
        PreferenceManager.getDefaultSharedPreferences(nexzoIdeAppLoader.getContext());

    row.verticalGap = getPrefInt(sharedPref, "Vertical", 1);

    int ramin = nexzoIdeAppLoader.getContext().getResources().getConfiguration().orientation;
    if (ramin == Configuration.ORIENTATION_LANDSCAPE) {
      if (PreferenceManager.getDefaultSharedPreferences(nexzoIdeAppLoader.getContext())
