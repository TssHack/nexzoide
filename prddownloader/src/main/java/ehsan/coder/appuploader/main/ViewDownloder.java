package ehsan.coder.appuploader.main;
...
public class ViewDownloder extends RelativeLayout {
  ...
  public void setDownload(String url, String name) {
    ...
    File file = new File("/storage/emulated/0/nexzoWebIDE/apk/" + name);
    if (file.exists()) {
      ...
      id =
          PRDownloader.download(url, "/storage/emulated/0/nexzoWebIDE/apk/", name)
              .build()
  ...
  void installApk(String name) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O
      ...
      intent.setData(Uri.parse("package:" + "ir.ehsancoder.nexzoide"));
      getContext().startActivity(intent);
    } else {
      ApkInstallerCompat compat =
          new ApkInstallerCompat(
              getContext(), new File("/storage/emulated/0/nexzoWebIDE/apk/"+ name));
      compat.execute();
    }
  }
}
